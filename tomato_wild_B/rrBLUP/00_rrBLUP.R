...
dir <- "/winmounts/tong/winhome/mpidir/11.TAGWAS/GS_all/tomato_wild_new/"
setwd(dir)
...
rrBLUP_pop <- function(xx,name){
  ...
  for (s in c(1:7)){
    ...
    trset_name <- set_all[-c(1:(nrow(set_all)-2*nwild[s+1])),]
    teset_name <- set_all[c(1:(nrow(set_all)-2*nwild[s+1])),]

    ypred_all <- matrix(NA,(nrow(set_all)-2*nwild[s+1]),nrep)
...
ability_regression_pop <- function(name,testgroup){
  ...
  for (s in c(1:7)){
    ...
    if (testgroup =="wild"){
      if (s != 7){
        ...
        ypred_name <- c(1:(length(wild)-nwild[s+1]))
      } else {
        teset_name <- set_all[NULL,]
        ypred_name <- NULL
      }
    }
    if (testgroup =="all"){
      teset_name <- set_all[c(1:(nrow(set_all)-2*nwild[s+1])),]
      ypred_name <- c(1:(nrow(set_all)-2*nwild[s+1]))
    }
    if (testgroup =="modern"){
      teset_name <- set_all[c((length(wild)-nwild[s+1]+1):(nrow(set_all)-2*nwild[s+1])),]
      ypred_name <- c((length(wild)-nwild[s+1]+1):(nrow(set_all)-2*nwild[s+1]))
    }
    ...
    write.table(cor1allm,paste("rrBLUP/pearson_",name,"_wild_",testgroup,"_",nwild[s+1],".csv",sep=""),sep=",",row.names=F,col.names=F)
    write.table(cor2allm,paste("rrBLUP/spearman_",name,"_wild_",testgroup,"_",nwild[s+1],".csv",sep=""),sep=",",row.names=F,col.names=F)
    write.table(match1allm,paste("rrBLUP/coincidence30_",name,"_wild_",testgroup,"_",nwild[s+1],".csv",sep=""),sep=",",row.names=F,col.names=F)
    write.table(match2allm,paste("rrBLUP/coincidence15_",name,"_wild_",testgroup,"_",nwild[s+1],".csv",sep=""),sep=",",row.names=F,col.names=F)
    ...
    write.table(results,paste("rrBLUP/summary_",name,"_wild_",testgroup,"_",nwild[s+1],".csv",sep=""),sep=",",row.names=F,col.names=T)
    ...
    write.table(resultsall_final,paste("rrBLUP/00_summary_",name,"_",testgroup,"_all.csv",sep=""),sep=",",row.names=F,col.names=T)
...
ability_regression_pop(paste0("tagsnp.",p),"wild")
ability_regression_pop(paste0("tagsnp.",p),"all")
ability_regression_pop(paste0("tagsnp.",p),"modern")
