...
nwild <- c(7,5*c(2:6),34)

for (i in c(1:7)){
  
  trset_all <- matrix(NA,(2*nwild[i]),nrepf)
  teset_all <- matrix(NA,(length(wild)+length(modern)-2*nwild[i]),nrepf)
  for (r in 1:nrepf){
    ...
    # sample the same size in modern accessions
    trset.modern <- sample(modern,nwild[i],replace=FALSE)
    
    # combine with modern accessions
    trset_name <- c(trset.modern,trset.wild_s_all,trset.wild_add)
    teset_name_wild <- setdiff(wild,c(trset.wild_s_all,trset.wild_add))
    teset_name_modern <- setdiff(modern,trset.modern)
    trset_all[,r] <- trset_name
    # first wild testing set, then modern testing set!! 
    teset_all[,r] <- c(teset_name_wild,teset_name_modern)
  }
