...
## load data
# genotypic data
data <- read.table("rrBLUP/00_summary_tagsnp.10_all.csv",head=TRUE,sep=",")[1:47,]
...
for (i in 1:7){
  ...
  output[length(group_u)+1,i] <- mean(datai)
  output[length(group_u)+1,i+7] <- sd(datai)
}

rownames(output) <- c(group_u,"All")
write.table(output,"rrBLUP/00_summary_tagsnp.10_all_group.csv",sep=",",row.names=T,col.names=F)
