...
## load data
# genotypic data
myY <- read.table("data/tomato_pheno_all.csv",head=TRUE,sep=",")
pheno <- myY[,-1]
...
for (s in c(1:6)){
  ...
  trset_all <- matrix(NA,(length(modern)+5*s),nrepf)
  teset_all <- matrix(NA,(length(wild)-5*s),nrepf)
  for (r in 1:nrepf){
    
    nwild <- 5*s
    trset.wild <- sample(wild,nwild,replace=FALSE)
    trset_name <- c(modern,trset.wild)
    teset_name <- setdiff(wild,trset.wild)
    trset_all[,r] <- trset_name
    teset_all[,r] <- teset_name
  }
  
  set_all <- rbind(teset_all,trset_all)
  write.table(set_all,paste("fold/testid_wild_",5*s,".csv",sep=""),sep=",",row.names=F,col.names=F)
...
rrBLUP_pop <- function(xx,name){
  ...
  for (s in c(0:6)){
    ...
    if (s == 0){
      ...
      nrep <- nrepf
      set_all <- read.table(paste("fold/testid_wild_",5*s,".csv",sep=""),head=F,sep=",")
      trset_name <- set_all[-c(1:(length(wild)-5*s)),]
      teset_name <- set_all[c(1:(length(wild)-5*s)),]
    }
    
    ypred_all <- matrix(NA,(length(wild)-5*s),nrep)
    for (n in 1:(ncol(pheno)-3)){
      ...
      write.table(ypred_all,paste("rrBLUP/predict_",name,"_trait_",n,"_wild_",5*s,".csv",sep=""),sep=",",row.names=F,col.names=F)
    }
...
ability_regression_pop <- function(name){
  ...
  for (s in c(0:6)){
    ...
    for (n in 1:(ncol(pheno)-3)){
      ...
      Ypred <- read.table(paste("rrBLUP/predict_",name,"_trait_",n,"_wild_",5*s,".csv",sep=""),head=F,sep=",")
      Y <- as.numeric(pheno[,n])
      cor1p <- NULL
      cor2p <- NULL
      match1p <- NULL
    ...
    write.table(cor1allm,paste("rrBLUP/pearson_",name,"_wild_",5*s,".csv",sep=""),sep=",",row.names=F,col.names=F)
    write.table(cor2allm,paste("rrBLUP/spearman_",name,"_wild_",5*s,".csv",sep=""),sep=",",row.names=F,col.names=F)
    write.table(match1allm,paste("rrBLUP/coincidence30_",name,"_wild_",5*s,".csv",sep=""),sep=",",row.names=F,col.names=F)
    write.table(match2allm,paste("rrBLUP/coincidence15_",name,"_wild_",5*s,".csv",sep=""),sep=",",row.names=F,col.names=F)
    ...
    write.table(results,paste("rrBLUP/summary_",name,"_wild_",5*s,".csv",sep=""),sep=",",row.names=F,col.names=T)
