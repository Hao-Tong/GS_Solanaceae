...
##########################################################################
# multiple traits
mt <- read.table("GS_MT/tomato/multi_trait.csv",sep=",",header=F)

# single trait predictability 
stdata <- read.table("GS/tomato/rrBLUP/pearson_tagsnp.10.csv",head=F,sep=",")
...
for (i in 1:nrow(mt)){
  ...
  mtdata <- read.table(paste0("GS_MT/tomato/results/pearson_tagsnp.10_",i,".csv"),head=F,sep=",")
  mtn <- as.numeric(mt[i,])
  mtn <- mtn[!is.na(mtn)]
...
write.table(pvalall,"GS_MT/tomato/results/ttest.csv",sep=",",row.names=F,col.names=F)
