...
# phenotypic data
myY_CD1 <- read.table("data/Tomato.AG.csv",head=TRUE,sep=",")
myY_CD2 <- read.table("data/Tomato.AG.add.csv",head=TRUE,sep=",")
myY_CD <- rbind(myY_CD1,myY_CD2)
rownames(myY_CD) <- myY_CD[,1]
myY_CD <- myY_CD[,-1]
myY_CD <- myY_CD[modern,]
  ...
  for (n in 1:ncol(classpheno)){
    ...
    for (r in 1:rr){
      ...
      for (p in 1:f){
        ...
        rffit <- caret::train(ytr~., data=newtrdata, method="rf", metric="Accuracy",
            na.action=na.pass, trControl=tcontr, tuneGrid=expand.grid(.mtry=sqrt(ncol(xxtr))),ntree=1000)
        ...
        ypredp <- predict(rffit,newdata=xxte,type="prob")
        ypred <- predict(rffit,newdata=xxte)
    ...
    write.table(ypred_all,paste("RF_CD/predict_",name,"_trait_",n,".csv",sep=""),sep=",",row.names=F,col.names=F)
    write.table(ypredp_all,paste("RF_CD/predict_probability_",name,"_trait_",n,".csv",sep=""),sep=",",row.names=F,col.names=F)
...
ability_classification <- function(name){
  ...
  for (n in 1:ncol(classpheno)){
    ...
    print(paste("Phenotype ",n," Start!",sep=""))
    Ypred <- read.table(paste("RF_CD/predict_",name,"_trait_",n,".csv",sep=""),head=F,sep=",")
    Ypredp <- read.table(paste("RF_CD/predict_probability_",name,"_trait_",n,".csv",sep=""),head=F,sep=",")
    Y <- classpheno[,n]
    Y[which(Y != "NA")] <- paste0("s",Y[which(Y != "NA")])
    nle <- nlevels(as.factor(Y))
  ...
  write.table(aucallm,paste("RF_CD/auc_",name,".csv",sep=""),sep=",",row.names=F,col.names=F)
  write.table(accallm,paste("RF_CD/accuracy_",name,".csv",sep=""),sep=",",row.names=F,col.names=F)
  write.table(match1allm,paste("RF_CD/coincidence1_",name,".csv",sep=""),sep=",",row.names=F,col.names=F)
  write.table(match2allm,paste("RF_CD/coincidence2_",name,".csv",sep=""),sep=",",row.names=F,col.names=F)
  ...
  write.table(results,paste("RF_CD/summary_",name,".csv",sep=""),sep=",",row.names=F,col.names=T)
...
# run the GS model
RF(as.matrix(genotag),paste0("tagsnp.",p))
